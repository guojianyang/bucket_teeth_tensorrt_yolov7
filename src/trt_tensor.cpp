...
namespace TRT{
	...
	void* MixMemory::gpu(size_t size) {
		...
		if (gpu_size_ < size) {
			...
			gpu_size_ = size;
			CUDATools::AutoDevice auto_device_exchange(device_id_);// 确保cuda操作在正确的设备上
	...
	void MixMemory::release_cpu() {
		if (cpu_) {
			if(owner_cpu_){
				CUDATools::AutoDevice auto_device_exchange(device_id_);//确保cuda操作在正确的设备上执行
	...
	Tensor& Tensor::release() {
		...
		if(stream_owner_ && stream_ != nullptr){
			CUDATools::AutoDevice auto_device_exchange(this->device());
			if(shape_.size() == 4){
				checkCudaRuntime(cudaStreamDestroy(stream_)); // 如果多个tensor示例调用同一个stream，此位置会出现重复释放的可能性，必须甄别！！！	
			}
			shape_.clear();
