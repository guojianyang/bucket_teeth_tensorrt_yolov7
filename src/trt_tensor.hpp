...
namespace TRT {
    ...
    class Tensor {
    public:
        Tensor(const Tensor& other) = delete; // 禁止Tensor对象的拷贝构造
        ...
        inline int size(int index)  const{return shape_[index];} // 推理用到
        inline int shape(int index) const{return shape_[index];} //推理用到，size和shape是为了多种“形状”定义的收录
        ...
        inline DataType type()                const { return dtype_; } //推理用到
        ...
        Tensor& resize(int ndims, const int* dims);     //推理用到
        Tensor& resize(const std::vector<int>& dims);   //推理用到
        Tensor& resize_single_dim(int idim, int size);  //推理用到
        int  count(int start_axis = 0) const;
        int device() const{return device_id_;}
        Tensor& to_gpu(bool copy=true);  //推理用到
        Tensor& to_cpu(bool copy=true);  //推理用到

        Tensor& to_half();
        Tensor& to_float();
        inline void* cpu() const { ((Tensor*)this)->to_cpu(); return data_->cpu(); } // 推理用到
        inline void* gpu() const { ((Tensor*)this)->to_gpu(); return data_->gpu(); } //推理用到
        ...
        inline DType* cpu(int i, _Args&& ... args) { return cpu<DType>() + offset(i, args...); } // 推理用到


        template<typename DType> inline const DType* gpu() const { return (DType*)gpu(); }  //推理用到
        template<typename DType> inline DType* gpu()             { return (DType*)gpu(); }  //推理用到

        template<typename DType, typename ... _Args> 
        inline DType* gpu(int i, _Args&& ... args) { return gpu<DType>() + offset(i, args...); } //推理用到
        ...
        std::shared_ptr<MixMemory> get_workspace()        const {return workspace_;} //推理用到
        ...
        Tensor& set_stream(CUStream stream, bool owner=false){stream_ = stream; stream_owner_ = owner; return *this;} //推理用到
        ...
        Tensor& synchronize();
        const char* shape_string() const{return shape_string_;} //推理用到
        const char* descriptor() const;

        Tensor& copy_from_gpu(size_t offset, const void* src, size_t num_element, int device_id = CURRENT_DEVICE_ID); //推理用到
