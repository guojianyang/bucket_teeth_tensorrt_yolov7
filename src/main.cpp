...
void inference_cuda_decode_nms_tensor(){
    ...
    cv::imwrite("after_cuda_post_progress.jpg", image);
    printf("已生成 after_cuda_post_progress.jpg, 储存在/workspace下\n ");
}
...
int main(){
    ...
    // inference();
    // 结论：经过decode和nms的cuda加速处理后，相比非cuda加速的decode和nms，延时降低7-8倍，加入标准库
